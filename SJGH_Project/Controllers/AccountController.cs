    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            ...
            if (Roles.GetRolesForUser(model.UserName)[0] == "patient")
            {
                ...
                status = Convert.ToInt32(objPatient.getPatientByUserName(model.UserName).status);
            }
            else if (Roles.GetRolesForUser(model.UserName)[0] == "doctor")
            {
                doctorClass objPatient = new doctorClass();
                status = Convert.ToInt32(objPatient.getDoctorByUserName(model.UserName).status);
            }
            ...
            if (ModelState.IsValid && WebSecurity.Login(model.UserName, model.Password, persistCookie: model.RememberMe) && status == 1)
            {
